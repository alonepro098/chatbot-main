...
async def cb_handler(_, query: CallbackQuery):
    if query.data == "HELP":
        ...
        if RADHE is None:
            return await query.answer("⚠️ Database is not connected!", show_alert=True)

        is_RADHE = await RADHE.find_one({"chat_id": query.message.chat.id})
        if not is_RADHE:
            await query.edit_message_text("**ᴄʜᴀᴛ-ʙᴏᴛ ᴀʟʀᴇᴀᴅʏ ᴇɴᴀʙʟᴇᴅ.**")
        else:
            await RADHE.delete_one({"chat_id": query.message.chat.id})
        ...
        is_RADHE = await RADHE.find_one({"chat_id": query.message.chat.id})
        if not is_RADHE:
            await RADHE.insert_one({"chat_id": query.message.chat.id})
            await query.edit_message_text(

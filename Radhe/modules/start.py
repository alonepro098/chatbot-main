...
from Radhe.database.chats import add_served_chat
from Radhe.database.users import add_served_user
from Radhe.modules.helpers import (
    CLOSE_BTN,
    DEV_OP,
    HELP_BTN,
    HELP_BUTN,
...
async def start(_, m: Message):
    if m.chat.type == ChatType.PRIVATE:
        ...
        await m.reply_photo(
            ...
            caption=(
                f"**๏ ʜᴇʏ, ɪ ᴀᴍ {Radhe.name}**\n"
                f"**➻ ᴀɴ ᴀɪ ʙᴀsᴇᴅ ᴄʜᴀᴛʙᴏᴛ.**\n"
                f"**──────────────**\n"
                f"**➻ ᴜsᴀɢᴇ /chatbot [ᴏɴ/ᴏғғ]**\n"
                f"<b>||๏ ʜɪᴛ ʜᴇʟᴘ ʙᴜᴛᴛᴏɴ ғᴏʀ ʜᴇʟᴘ||</b>"
            ),
...
@Radhe.on_cmd("help")
async def help(client: Radhe, m: Message):
    if m.chat.type == ChatType.PRIVATE:
        await m.reply_photo(
            photo=random.choice(IMG),
            caption=HELP_READ,
...
@Radhe.on_message(filters.new_chat_members)
